...
@section('title', 'Create Role - Tixx Accounts ERP')
...
<div class="role-form-page-wrapper">
    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4 pb-2 border-bottom">
        ...
        <div class="d-flex align-items-center gap-2">
            ...
            <button type="submit" form="roleCreateForm" class="btn btn-primary btn-sm rounded-3 px-3 py-2 fw-semibold shadow-sm">
    ...
    <div id="unsavedChangesWarning" class="alert alert-warning d-none align-items-center justify-content-between rounded-3 mb-4 py-2 px-3">
        ...
        <button type="submit" form="roleCreateForm" class="btn btn-warning btn-sm fw-bold">Save Changes Now</button>
    </div>
    ...
    <form id="roleCreateForm" action="{{ route('roles.store') }}" method="POST">
            ...
            <div class="profile-card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="status" id="status" class="form-select profile-custom-input">
                    ...
                    <div class="col-xl-6 col-lg-12 permission-module-card-col" data-module="{{ strtolower($moduleName) }}">
                        <div class="erp-card h-100 border shadow-sm permission-module-card">
                            ...
                            <div class="row g-2">
                                ...
                                @php
                                    $isPreChecked = isset($assignedPermissionIds) && in_array($permission->id, $assignedPermissionIds);
                                @endphp
        ...
        <div class="d-flex align-items-center justify-content-end gap-2 mb-5">
            <a href="{{ route('roles.index') }}" class="btn btn-light border px-4 py-2 fw-semibold text-secondary rounded-3">
                Cancel
            </a>
            <button type="submit" class="btn btn-primary px-4 py-2 fw-semibold rounded-3 shadow-sm">
                <i class="fa-solid fa-check me-2"></i> Save Role & Permissions
            </button>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const roleSlugInput = document.getElementById('roleSlugInput');
    ...
    if (roleNameInput && roleSlugInput) {
        roleNameInput.addEventListener('input', function () {
            if (!roleSlugInput.dataset.manual) {
                roleSlugInput.value = this.value
                    .toLowerCase()
                    .replace(/[^a-z0-9 -]/g, '')
                    .replace(/\s+/g, '-')
                    .replace(/-+/g, '-');
